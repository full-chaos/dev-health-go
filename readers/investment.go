...
WHERE rn = 1`, DefaultRowLimit)
	...
	err := QueryOrgScoped(ctx, client, "ReadTeamInvestment", statement, orgID, ids, func(row RowScanner) error {
...
ORDER BY p.id, tpo.team_id, im.investment_area, im.project_stream`, DefaultRowLimit)
	...
	err := QueryOrgScoped(ctx, client, "ReadProjectInvestment", statement, orgID, ids, func(row RowScanner) error {
